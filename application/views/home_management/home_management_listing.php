...
<div class="grid grid-cols-12 gap-6 mt-5">
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-no-wrap items-center mt-2">
        <!-- <a class="button text-white bg-theme-1 shadow-md mr-2" href="<?php //echo base_url(); ?>addhome_management">Add New</a> -->
        <!-- <div class="dropdown">
        ...
        <div class="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
            <div class="w-56 relative text-gray-700 dark:text-gray-300">
                <form action="<?php echo base_url() ?>home_management_listing" method="POST" id="searchList">
        ...
        <table class="table table-report -mt-2">
            <thead>
                <tr>
                    ...
                    <th class="whitespace-no-wrap">SUBTITLE 1</th>
                    <th class="whitespace-no-wrap">SUBTITLE 2</th>
            ...
            <tbody>
                <?php if(!empty($home_managements)) { $i=1; foreach($home_managements as $home_management) { ?>
                <tr class="intro-x">
                    <td><?php echo $i ?></td>
                    <td>
                        <a href="" class="font-medium whitespace-no-wrap"><?php echo $home_management->title ?></a>
                    </td>
                    <td class=""><?php echo $home_management->subtitle_one ?></td>
                    <td class=""><?php echo $home_management->subtitle_two ?></td>
                    <td class="text-justify"><?php echo $home_management->description ?></td>
                    <td class="w-40">
                        <?php if(!empty($home_management->image)) { ?>
                        <div class="flex">
                            <div class="w-full h-20 my-5 image-fit">
                                ...
                                <img alt="Image" data-action="zoom" class="w-full tooltip rounded-full" src="<?php echo assets_url(); ?>uploads/<?php echo $home_management->image; ?>">
                            </div>
                        </div>
                        <?php } else { ?>
                        <div class="flex">
                    ...
                    <td class="table-report__action w-56">
                        <div class="flex justify-center items-center">
                            <a class="flex items-center mr-3 text-theme-10" href="<?php echo base_url().'edithome_management/'.$home_management->home_management_id; ?>"> <i data-feather="check-square" class="w-4 h-4 mr-1"></i> Edit </a>
                            <!-- <a class="flex items-center text-theme-6" href="javascript:;" data-toggle="modal" data-target="#delete-confirmation-modal" onclick="read_more_home_management(<?php //echo $home_management->home_management_id; ?>)"> <i data-feather="trash-2" class="w-4 h-4 mr-1"></i> Delete </a> -->
                        </div>
                    </td>
                </tr>
                <?php $i++; } } ?>
...
<div class="modal" id="delete-confirmation-modal">
    <div class="modal__content">
        ...
        <div class="px-5 pb-8 text-center" id="home_management_delete_data">
...
<script type="text/javascript">
    ...
    //         jQuery("#searchList").attr("action", baseURL + "home_management_listing/" + value);
    //         jQuery("#searchList").submit();
    //     });
    // });


    function read_more_home_management(id)
    {
        $.ajax({
           type: 'POST',
           url: '<?php echo base_url().'home_management/home_management_details'?>',
           data: {'home_management_id':id},
           // dataType: "JSON",
           success: function(response) {
                data = JSON.parse(response);
                var html="";
                html+='<button type="button" data-dismiss="modal" class="button w-24 border text-gray-700 mr-1">Cancel</button>';
                html+='<a type="button" href="<?php echo base_url() ?>deletehome_management/'+data.home_management_id+'" class="button w-24 bg-theme-6 text-white">Delete</a>';
                $("#home_management_delete_data").html(html);
            }
        });
    }
</script>