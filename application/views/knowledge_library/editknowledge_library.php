<?php
$knowledge_library_id = $knowledge_libraryInfo->knowledge_library_id;
$content_preferenceID = $knowledge_libraryInfo->content_preferenceID;

// dd($array_exelevelID);
// dd($array_simplicityID);

$title = $knowledge_libraryInfo->title;
$preference = $knowledge_libraryInfo->preference;
$image = $knowledge_libraryInfo->image;
$time = $knowledge_libraryInfo->time;
$description = $knowledge_libraryInfo->description;
?>
        ...
        <div class="intro-y box">
            ...
            <div class="p-5" id="form-validation">
                <div class="preview">
                    ...
                    <form role="form" class="validate-form" id="knowledge_library" enctype='multipart/form-data' action="<?php echo base_url() ?>editknowledge_libraryConfig" method="post">
                        <div class="grid grid-cols-2 gap-2">
                            <div class="input-form mt-3">
                                ...
                                <input type="hidden" value="<?php echo $knowledge_library_id; ?>" name="knowledge_library_id" id="knowledge_library_id" />
                            </div>
                            <div class="input-form mt-3">
                                <label for="time" class="flex flex-col sm:flex-row"> Time <span class="sm:ml-1 mt-1 sm:mt-0 text-red-600">*</span> </label>
                                <input type="text" name="time" id="time" value="<?php echo $time; ?>" class="input w-full border col-span-4 required" placeholder="John Legend" required>
                            </div>
                        </div>
                        <div class="grid grid-cols-0 gap-2">
                            <div class="input-form mt-3">
                                ...
                                <select name="preference" id="preference" value="<?php echo set_value('preference'); ?>" data-preference="" class="input tail-select w-full border col-span-4 " placeholder="John Legend" onchange="myFunction(this.value);" selected> 
                                    <option value="<?=$preference?>"><?php if($preference==1){echo "Video";} if($preference==2){echo "Image";} ?></option>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <?php if($preference==1){ $style = "";} else { $style = "display: none;"; }?>
                            <div class="input-form mt-3 box" id="images" style="<?=$style?>">
                                <label for="image_files" class="flex flex-col sm:flex-row"> Video <span class="sm:ml-1 mt-1 sm:mt-0 text-red-600">*</span> </label>
                                <input type="url" name="image" id="image" value="<?php echo $image; ?>" class="input w-full border col-span-4" placeholder="John Legend">
                            </div>
                            <?php if($preference==2){ $style = "";} else { $style = "display: none;"; }?>
                            <div class="input-form mt-3" id="image_filess" style="<?=$style?>">
                                ...
                                <!-- <div data-single="true" action="<?php //echo base_url(); ?>uploads" class="dropzone">
                                    <div class="fallback">
                                        <input type="file" name="image_files" id="image_files" value="<?php //echo $image_files; ?>" class="input w-full border mt-2" required>
                                    </div>
                                ...
                                <ul class="pagination">
                                <?php if(!empty($getknowledge_library_multi_fileInfo)){ foreach($getknowledge_library_multi_fileInfo as $multi_file){ if($multi_file->content_preference_imageID == 2){ ?>
                                    <li><img alt="Image" data-action="zoom" class="" src="<?php echo assets_url(); ?>uploads/<?php echo $multi_file->image_files; ?>" width="100" height="100"></li>
                                <?php } } } else { ?>
                                </ul>
                        ...
                        <div class="grid grid-cols-0 gap-2">
                            <div class="input-form mt-3">
                                ...
                                <textarea class="input w-full border mt-2 editor" id="description" value="" name="description" placeholder="Type your text here" required><?php echo $description; ?></textarea>
                            </div>
                        </div>
                        <div class="grid grid-cols-3 gap-2">
                            <div class="input-form mt-3">
                                ...
                                <div class="flex items-center text-gray-700 dark:text-gray-500 mt-2">
                                    <input type="radio" name="content_preferenceID" id="content_preferenceID" class="input border mr-2" value="<?=$content_preference->content_preference_id?>" <?php echo ($content_preferenceID==$content_preference->content_preference_id)?'checked':'' ?>> 
                            ...
                            <div class="input-form mt-3">
                                ...
                                <div class="flex items-center text-gray-700 dark:text-gray-500 mt-2"> 
                                    <input type="checkbox" class="input border mr-2" name="catID[]" id="catID" value="<?=$categories->categories_id?>" required <?php if(in_array($categories->categories_id, $array_catID)) echo "checked" ?>> 
                            ...
                            <div class="input-form mt-3" style="display: none;">
                                ...
                                <div class="flex items-center text-gray-700 dark:text-gray-500 mt-2"> 
                                    <input type="checkbox" class="input border mr-2" name="exelevelID[]" id="exelevelID" value="<?=$level->level_id?>" required <?php if(in_array($level->level_id, $array_exelevelID)) echo "checked" ?>> 
