...
<div class="grid grid-cols-12 gap-6 mt-5">
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-no-wrap items-center mt-2">
        <a class="button text-white bg-theme-1 shadow-md mr-2" href="<?php echo base_url(); ?>addfitness_recipe">Add New Recipe</a>
        <!-- <div class="dropdown">
        ...
        <div class="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
            <div class="w-56 relative text-gray-700 dark:text-gray-300">
                <form action="<?php echo base_url() ?>fitness_recipe_listing" method="POST" id="searchList">
    ...
    <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
        <table class="table table-report -mt-2">
            ...
            <tbody>
                <?php if(!empty($fitness_recipes)) { $i=1; foreach($fitness_recipes as $fitness_recipe) { ?>
                <tr class="intro-x">
                    <td><?php echo $i ?></td>
                    <td>
                        <a href="" class="font-medium whitespace-no-wrap"><?php echo $fitness_recipe->title ?></a>
                    </td>
                    <td class="w-40">
                        <div class="flex">
                            <div class="w-full h-20 my-5 image-fit">
                                ...
                                <img alt="Image" data-action="zoom" class="w-full tooltip rounded-full" src="<?php echo assets_url(); ?>uploads/<?php echo $fitness_recipe->image; ?>" title="<?php echo $fitness_recipe->title ?>">
                            </div>
                        </div>
                    </td>
                    <td class="table-report__action w-56">
                        <div class="flex justify-center items-center">
                            <a class="flex items-center mr-3 text-theme-10" href="<?php echo base_url().'editfitness_recipe/'.$fitness_recipe->fitness_recipe_id; ?>"> <i data-feather="check-square" class="w-4 h-4 mr-1"></i> Edit </a>
                            <a class="flex items-center text-theme-6" href="javascript:;" data-toggle="modal" data-target="#delete-confirmation-modal" onclick="read_more_fitness_recipe(<?php echo $fitness_recipe->fitness_recipe_id; ?>)"> <i data-feather="trash-2" class="w-4 h-4 mr-1"></i> Delete </a>
                        </div>
                    </td>
                </tr>
                <?php $i++; } } ?>
...
<div class="modal" id="delete-confirmation-modal">
    <div class="modal__content">
        ...
        <div class="px-5 pb-8 text-center" id="fitness_recipe_delete_data">
...
<script type="text/javascript">
    ...
    //         jQuery("#searchList").attr("action", baseURL + "fitness_recipe_listing/" + value);
    //         jQuery("#searchList").submit();
    //     });
    // });


    function read_more_fitness_recipe(id)
    {
        $.ajax({
           type: 'POST',
           url: '<?php echo base_url().'fitness_recipe/fitness_recipe_details'?>',
           data: {'fitness_recipe_id':id},
           // dataType: "JSON",
           success: function(response) {
                data = JSON.parse(response);
                var html="";
                html+='<button type="button" data-dismiss="modal" class="button w-24 border text-gray-700 mr-1">Cancel</button>';
                html+='<a type="button" href="<?php echo base_url() ?>deletefitness_recipe/'+data.fitness_recipe_id+'" class="button w-24 bg-theme-6 text-white">Delete</a>';
                $("#fitness_recipe_delete_data").html(html);
            }
        });
    }
</script>