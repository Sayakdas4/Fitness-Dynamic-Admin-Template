<?php
$exercise_video_id = $exercise_videoInfo->exercise_video_id;
$body_partID = $exercise_videoInfo->body_partID;
$equipmentID = $exercise_videoInfo->equipmentID;

// dd($array_exelevelID);
// dd($array_simplicityID);

$title = $exercise_videoInfo->title;
$image = $exercise_videoInfo->image;
$description = $exercise_videoInfo->description;
?>
        ...
        <div class="intro-y box">
            ...
            <div class="p-5" id="form-validation">
                <div class="preview">
                    ...
                    <form role="form" class="validate-form" id="exercise_video" enctype='multipart/form-data' action="<?php echo base_url() ?>editexercise_videoConfig" method="post">
                        <div class="grid grid-cols-2 gap-2">
                            <div class="input-form mt-3">
                                ...
                                <input type="hidden" value="<?php echo $exercise_video_id; ?>" name="exercise_video_id" id="exercise_video_id" />
                                <!-- <input type="hidden" value="<?php //echo $exercise_video_check_box_id; ?>" name="exercise_video_check_box_id" id="exercise_video_check_box_id" /> -->
                            </div>
                            <div class="input-form mt-3">
                                <label for="image" class="flex flex-col sm:flex-row"> Video <span class="sm:ml-1 mt-1 sm:mt-0 text-red-600">*</span> </label>
                                <input type="url" name="image" id="image" value="<?php echo $image; ?>" class="input w-full border col-span-4 required" placeholder="John Legend" required>
                                <iframe width="560" height="315" src="<?=$image?>" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                            </div>
                        </div>
                        <div class="grid grid-cols-0 gap-2">
                            <div class="input-form mt-3">
                                ...
                                <textarea class="input w-full border mt-2" id="description" value="" name="description" placeholder="Type your text here" required><?php echo $description; ?></textarea>
                            </div>
                        </div>
                        <div class="grid grid-cols-3 gap-2">
                            <div class="input-form mt-3">
                                ...
                                <div class="flex items-center text-gray-700 dark:text-gray-500 mt-2">
                                    <input type="radio" name="body_partID" id="body_partID" class="input border mr-2" value="<?=$body_part->body_part_id?>" <?php echo ($body_partID==$body_part->body_part_id)?'checked':'' ?>> 
                                    <label class="cursor-pointer select-none" for="horizontal-radio-chris-evans"><?=$body_part->body_part_title?></label> 
                                </div>
                                <?php } } ?>
                            </div>
                            <div class="input-form mt-3">
                                <label for="equipmentID" class="flex flex-col sm:flex-row"> Recipe Type <span class="sm:ml-1 mt-1 sm:mt-0 text-red-600">*</span> </label>
                                <?php if(!empty($equipments)){ foreach($equipments as $equipment){ ?>
                                <div class="flex items-center text-gray-700 dark:text-gray-500 mt-2">
                                    <input type="radio" name="equipmentID" id="equipmentID" class="input border mr-2" value="<?=$equipment->equipment_id?>" <?php echo ($equipmentID==$equipment->equipment_id)?'checked':'' ?>> 
